...
import type { FilterFieldDef, PermissionGrants } from '@erp/shared';
...
import { Badge, Combobox, ConfirmDialog, Field, Modal, Select, Spinner, Switch, Tabs, TextInput } from '@/components/ui';
...
const defaults = { firstName: '', lastName: '', email: '', username: '', mobile: '', password: '', roleId: '', companyIds: [] as string[], branchIds: [] as string[], permissionOverrides: {} as PermissionGrants, isActive: true, customFields: {} as Record<string, any> };
...
function UserForm({ open, onClose, row }: { open: boolean; onClose: () => void; row?: any | null }) {
  ...
  useEffect(() => { if (open) { setTab('details'); reset(row ? { ...defaults, ...row, username: row.username ?? '', mobile: row.mobile ?? '', password: '', permissionOverrides: row.permissionOverrides ?? {}, customFields: row.customFields ?? {} } : { ...defaults, roleId: roles.data?.find((r) => r.key === 'user')?.id ?? '' }); } }, [open, row, reset]); // eslint-disable-line
  const companyIds = watch('companyIds');
  const roleId = watch('roleId');
  const role = useQuery({ queryKey: ['role', roleId], queryFn: () => api.get<any>(`/api/admin/roles/${roleId}`), enabled: open && !!roleId });
  ...
  const submit = handleSubmit((v) => {
    ...
    save.mutate({ method: row ? 'put' : 'post', url: row ? `/api/admin/users/${row.id}` : '/api/admin/users', body }, { onError: (e) => { applyApiErrors(e, setError as any); setTab('details'); } });
  });
  ...
  const isSuperRole = role.data?.key === 'super_admin';
  return (
    <Modal open={open} onClose={onClose} title={row ? `Edit User — ${row.name}` : 'Add User'} size="xl" footer={<><button className="btn-outline" onClick={onClose}>Cancel</button><button className="btn-primary" onClick={submit} disabled={save.isPending}>{save.isPending && <Spinner />} {row ? 'Update' : 'Save'}</button></>}>
      <Tabs value={tab} onChange={(v) => setTab(v as any)} tabs={[{ value: 'details', label: 'Details' }, { value: 'access', label: 'Extra Permissions' }]} className="mb-5" />
      <form onSubmit={submit} className={cx('grid gap-4 sm:grid-cols-2', tab !== 'details' && 'hidden')}>
        ...
        <Field label="Role" required error={errors.roleId?.message}><Controller control={control} name="roleId" rules={{ required: 'Role is required' }} render={({ field }) => <Select value={field.value} onChange={field.onChange} options={(roles.data ?? []).map((r) => ({ value: r.id, label: r.name }))} />} /></Field>
        ...
        <Field label="Company Access" hint="Leave empty for all companies"><Controller control={control} name="companyIds" render={({ field }) => <Combobox multiple value={field.value} onChange={field.onChange} options={(companies.data ?? []).map((c) => ({ value: c.id, label: c.name }))} />} /></Field>
        <Field label="Branch Access" hint="Leave empty for all branches"><Controller control={control} name="branchIds" render={({ field }) => <Combobox multiple value={field.value} onChange={field.onChange} options={branchOpts} />} /></Field>
      ...
      <div className={tab !== 'access' ? 'hidden' : ''}>
        {isSuperRole ? (
          ...
          <Controller control={control} name="permissionOverrides" render={({ field }) => <PermissionMatrix value={field.value} onChange={field.onChange} baseline={role.data?.permissions} />} />
        )}
      </div>
    </Modal>
  );
...
export default function UsersPage() {
  ...
  const filterFields: FilterFieldDef[] = [
    { key: 'firstName', label: 'First Name' }, { key: 'lastName', label: 'Last Name' }, { key: 'email', label: 'Email' }, { key: 'mobile', label: 'Mobile' },
  ...
  const columns: Column<any>[] = [
    ...
    { key: 'username', header: 'Username', render: (r) => r.username || '-', hidden: true },
